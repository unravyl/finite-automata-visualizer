...
const getConnectionPoint = (index: number, type: 'source' | 'target') => {
  // Use top or bottom connection point for the source node
  if (type === 'source') {
    return index % 2 === 0 ? 'bottom-source' : 'top-source';
  }
  
  // Use left or right connection point for the target node
...
const GraphSummary = (props: PropsInterface) => {
  ...
  const edges = validLinks.map((link, index) => {
    ...
    const isVerticalEdge = sourcePoint.includes('top') || sourcePoint.includes('bottom');

    // Calculate label position based on edge direction
